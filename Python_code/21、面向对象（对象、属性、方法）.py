...
class Teacher(object):
    ...
    def teach(self, course):  # 10、对象方法: 第一个参数位隐式的接收了**实例对象**
        print(f'{self.name}正在授{course}课')

    @classmethod  # 12、类方法装饰器，有这个装饰器就是类方法
    def teach(cls, course):  # 类方法: 第一个参数位隐式的接收了**类对象**
        print(f'{cls.school}的老师正在授{course}课')

    @staticmethod  # 13、静态方法装饰器
    def teach(course):
...
""" 13、类方法的调用规则: 既可以用类对象去调用(推荐), 也可以用实例对象去调用 """
# Teacher.teach('英语')
# teacher1.teach('数学')  # 本质上 type(teacher1).teach('数学')

""" 14、静态方法的调用规则: 既可以用类对象去调用(推荐), 也可以用实例对象去调用 """
# Teacher.teach('英语')
# teacher1.teach('数学')


"""11、 对象方法的调用规则: 只能用实例对象去调用 """
# teacher1.teach('数学')  # 等价于 Teacher.teach(teacher1, '数学')  不推荐
# teacher2.teach('语文')  # 等价于 Teacher.teach(teacher2, '数学')  不推荐

# 主动调用__init__方法，__init__初始化方法也可以看作是一个（会被__new__自动调用的对象方法
