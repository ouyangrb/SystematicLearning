...
class Teacher(object):
    ...
    # 方法: 类当中的函数
    # 魔术方法(特殊方法): 官方定义好的, 以两个下划线开头并且以两个下划线结尾的方法
...
"""
每次实例化时, 会自动调用__new__(cls)方法, 把要实例化的类对象和其他的实参打包, 其中类对象作
...
""" 类属性调用规则: 既可以用类对象去调用(推荐), 也可以用实例对象去调用
    当实例属性名和类属性名相同时, 实例对象优先调用实例属性 """
# print(getattr(Teacher, 'school', 'laoli'))
# print(Teacher.school)
# print(teacher1.school)  # 本质 print(type(teacher1).school)
# print(teacher2.school)  # 本质 print(type(teacher2).school)

""" 修改实例属性: 只能用实例对象访问到该属性, 再重新赋值 """
# print(teacher1.age)
# teacher1.age = 47
# print(teacher1.age)

""" 修改类属性: 只能用类对象访问到该属性, 再重新赋值 """
# print(Teacher.school)
# Teacher.school = '深兰教育'
# print(Teacher.school)
...
""" 动态定义实例属性: 当实例对象修改的属性不存在时, 会新增一个该实例属性 """
...
""" 动态定义类属性: 当类对象修改的属性不存在时, 会新增一个该类属性 """
